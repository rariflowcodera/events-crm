...
/**
 * Render English CTA button (outside blend mode).
 */
function renderEnglishCta(
  content: StructuredEmailContent,
  brandingContext: Record<string, string | boolean>
): string {
  const template = Handlebars.compile(englishCtaTemplate)
  return template({
    cta: content.cta,
    ...brandingContext,
  })
...
/**
 * Render Arabic CTA button (outside blend mode).
 */
function renderArabicCta(
  content: Partial<StructuredEmailContent>,
  brandingContext: Record<string, string | boolean>
): string {
  const template = Handlebars.compile(arabicCtaTemplate)
  return template({
    cta: content.cta,
    ...brandingContext,
  })
...
export function renderStructuredEmail(options: RenderOptions): RenderResult {
  ...
  // 6b. Render CTA sections (outside blend mode)
  const enCtaHtml = renderEnglishCta(processedEnContent, brandingContext)
  const arCtaHtml = processedArContent
    ? renderArabicCta(processedArContent, brandingContext)
    : ""
...
function processDocumentVariables(
  ...
  for (const doc of visibleDocs) {
    ...
    result = result.replace(customTextPattern, (_, customText) => {
      return `<a href="${brandedUrl}" style="color: #0066CC; -webkit-text-fill-color: #0066CC; text-decoration: underline;">${customText.trim()}</a>`
    })

    // Pattern 3: {{document.UUID}}
    const defaultPattern = new RegExp(`\\{\\{document\\.${doc.id}\\}\\}`, "g")
    result = result.replace(defaultPattern, `<a href="${brandedUrl}" style="color: #0066CC; -webkit-text-fill-color: #0066CC; text-decoration: underline;">${doc.name}</a>`)
  }
...
function processFormLinkVariables(
  ...
  for (const { form, token } of formTokens) {
    ...
    result = result.replace(customTextPattern, (_, customText) => {
      return `<a href="${formUrl}" style="color: #0066CC; -webkit-text-fill-color: #0066CC; text-decoration: underline;">${customText.trim()}</a>`
    })
...
function processVappLinkVariables(
  ...
  result = result.replace(/\{\{vapp\.link\|([^}]+)\}\}/g, (_, customText) => {
    return `<a href="${vappUrl}" style="color: #0066CC; -webkit-text-fill-color: #0066CC; text-decoration: underline;">${customText.trim()}</a>`
  })
...
function processHyperlinkVariables(html: string): string {
  ...
  result = result.replace(
    /\{\{hyperlink\|([^|]+)\|([^}]+)\}\}/g,
    (_, url, displayText) => `<a href="${url.trim()}" style="color: #0066CC; -webkit-text-fill-color: #0066CC; text-decoration: underline;">${displayText.trim()}</a>`
  )

  // Pattern 3: {{hyperlink|URL}} - link with URL as display text
  result = result.replace(
    /\{\{hyperlink\|([^}]+)\}\}/g,
    (_, url) => `<a href="${url.trim()}" style="color: #0066CC; -webkit-text-fill-color: #0066CC; text-decoration: underline;">${url.trim()}</a>`
  )
