...
export default async function VappCustomErrorPage({ params, searchParams }: ErrorPageProps) {
  const { locale: pathLocale } = await params
  const { reason, lang } = await searchParams

  // For custom domains, prefer lang query param over path locale
  const locale = lang || pathLocale
